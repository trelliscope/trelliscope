...
library(dplyr, warn.conflicts = FALSE)
library(lubridate, warn.conflicts = FALSE)
ds <- open_dataset("../examples/examples/12-bigdata/nyc-taxi") %>%
  mutate(
...
As discussed in the [Creating Panel Columns](panels.html) article, we can link the full dataset to this summary dataset with a plot function that will be applied to each subset of the full dataset, as defined by the variables that uniquely define a row in the summary dataset (in this case, year, month, and day). Conveniently, the full dataset can be an Arrow dataset, which is what we have here. This means that we do not need to read the huge input data into R to create the display, allowing us to build it nearly instantaneously.

<!-- ggplot(dsumm, aes(date, n)) + geom_line() -->

Our plot function takes as a single argument a data object which will represents the full dataset filtered to the subset that corresponds to the summary row being plotted.

We can get a test subset to work with using the following:

```r
x <- get_test_subset(ds, dsumm)
...
#> year: int32
#> month: int32
#> day: int64 (day(pickup_datetime))
#> hour: int64 (hour(pickup_datetime))
#> 
#> * Filter: (((year == 2009) and (month == 1)) and (day(pickup_datetime) == 1))
#> See $.data for the source Arrow object
```

Since the full dataset is an Arrow dataset, our test subset represents a query against this dataset. We see that this subset will represent the data for January 1, 2009. We can use Arrow to count the number of rides in each hour of the day and then collect the results to obtain the data that we want to plot.
