...
- `set_labels()`, `set_layout()`, `set_sort()`, `set_filters()`: specify the initial state of the appear
...
By default, the "key columns" will be shown as labels. If we'd like to change what labels are shown when the display is opened, we can use `set_labels()`, e.g.:
...
We can set the default filter state with `set_filters()`. Currently there are two different kinds of filters:
